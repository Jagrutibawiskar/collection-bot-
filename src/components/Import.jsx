...
export default function Import() {
  ...
  function nextStep() {
    if (step === 1) {
      saveImportMapping(importJobId, Object.fromEntries(mappings));
      runProgress("Saving mapping", "Mapping saved");
    }
    if (step === 2) {
      getImportJob(importJobId);
      runProgress("Validating rows", "Validation complete");
    }
    if (step === 3) {
      setComplete(true);
    ...
    <>
      ...
      <div className="panel import-panel">
        <div className="panel-body">
          ...
          <div className="import-status"><div><strong>{jobStatus}</strong><span>{importJobId}</span></div><div className="progress-track"><span style={{ width: `${progress}%` }} /></div>{progress > 0 && progress < 100 && <button type="button" className="ghost mini" onClick={cancelUpload}>Cancel</button>}</div>
          {cancelled && <div className="auth-alert error import-alert">Upload cancelled. Choose another file to continue.</div>}

          {step === 0 && <div className="grid dashboard-grid"><label className="dropzone"><input ref={inputRef} type="file" accept=".csv,.xlsx" onChange={pickFile} hidden /><div><strong>{file ? selectedName : "Drag a CSV or XLSX here"}</strong><p className="muted">Upload customer, EMI, paid, pending, fine, insurance and next due date columns.</p><button type="button" className="primary" onClick={() => inputRef.current?.click()}>Browse file</button></div></label><div className="import-summary-card"><span className="eyebrow">Upload safety</span><h3>Nothing sends until confirmation.</h3><p>Step 4 shows how many customers will be contacted today before campaigns start.</p><div className="import-option-row"><button type="button" className="ghost" onClick={downloadTemplate}>Download template</button><button type="button" className="ghost" onClick={useDemoFile}>Use sample file</button><button type="button" className="ghost" onClick={() => setJobStatus("SFTP source selected")}>Import from SFTP</button><button type="button" className="ghost" onClick={() => setJobStatus("API import selected")}>Use API</button></div></div></div>}

          {step === 1 && <div className="grid dashboard-grid"><div><h3 className="section-title">Column mapping</h3>{mappings.map(([from, to], index) => <div key={from} className="mapping-row"><strong>{from}</strong><select className="control" value={to} onChange={event => updateMapping(index, event.target.value)}>{fieldOptions.map(option => <option key={option}>{option}</option>)}</select><span>{to === "ignore" ? "○" : "✓"}</span></div>)}</div><div className={`auth-alert ${mappedRequired ? "ok" : "error"}`}>{mappedRequired ? "Required fields are mapped. You can continue to validation." : "Map customer_name, mobile, outstanding_amount and due_date before importing."}</div></div>}

          {step === 2 && <div><div className="import-result-grid"><Kpi label="Rows ready" value={readyRows.toLocaleString()} note="safe to import" /><Kpi label="Rows with problems" value={badRows.toLocaleString()} note="review before commit" /><Kpi label="Duplicates" value="12" note="policy required" /></div><div className="table-wrap import-issues"><table><thead><tr><th>Row</th><th>Field</th><th>Value</th><th>Issue</th></tr></thead><tbody>{validationRows.map(row => <tr key={row.row}><td>{row.row}</td><td>{row.field}</td><td>{row.value}</td><td>{row.reason}</td></tr>)}</tbody></table></div><div className="import-policy-grid"><div><span className="eyebrow">Bad rows</span><div className="segmented-control"><button type="button" className={badRowPolicy === "skip" ? "active" : ""} onClick={() => setBadRowPolicy("skip")}>Skip bad rows</button><button type="button" className={badRowPolicy === "fix" ? "active" : ""} onClick={() => setBadRowPolicy("fix")}>Fix here</button><button type="button" className={badRowPolicy === "cancel" ? "active" : ""} onClick={() => setBadRowPolicy("cancel")}>Cancel import</button></div></div><div><span className="eyebrow">Duplicates</span><div className="segmented-control"><button type="button" className={duplicatePolicy === "update" ? "active" : ""} onClick={() => setDuplicatePolicy("update")}>Update duplicates</button><button type="button" className={duplicatePolicy === "ignore" ? "active" : ""} onClick={() => setDuplicatePolicy("ignore")}>Ignore duplicates</button></div></div><button type="button" className="ghost" onClick={downloadRejectedRows}>Download rejected rows</button></div></div>}

          {step === 3 && <div className="confirm-panel"><span className="eyebrow">Confirm import</span><h3>Importing {readyRows.toLocaleString()} accounts to L&T Finance.</h3><p>{contactToday.toLocaleString()} will enter D-7/D-4 reminders today. 380 are already overdue and move to manager queue.</p><label className="checkbox-row"><input type="checkbox" checked={startCampaigns} onChange={event => setStartCampaigns(event.target.checked)} />Start campaigns immediately</label><div className="auth-alert ok">Bad rows: {badRowPolicy}. Duplicate policy: {duplicatePolicy === "update" ? "update existing accounts" : "ignore existing accounts"}.</div>{complete && <div className="auth-alert ok">Import job queued successfully. Polling /api/imports/:id for progress.</div>}</div>}

          <div className="wizard-actions"><button type="button" className="ghost" onClick={previousStep} disabled={step === 0}>Back</button><button type="button" className="primary" onClick={nextStep} disabled={(step === 1 && !mappedRequired) || (step === 2 && badRowPolicy === "cancel")}>{step === 3 ? "Import accounts" : "Continue"}</button></div>
        </div>
      </div>
    </>
  );
